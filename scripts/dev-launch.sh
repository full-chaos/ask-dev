...
#
# ACR_TARGET picks what the Workbench's server hop talks to (default: kiac):
#   - kiac   (default): the shared kiac k3s cluster's acr-pilot Helm release
#     (namespace/service "acr-pilot", real trial data, no local containers to
#     manage). Reached over a supervised `kubectl port-forward` to
#     127.0.0.1:18080 -- kubectl port-forward dies silently on a dropped
#     connection, so this script wraps it in a restart-forever loop instead
#     of a bare foreground command.
#   - docker: a real local ACR Context Fabric backend (FalkorDB + acr-projector
...
#     dev-health-clickhouse containers and their real seeded data. This does
#     NOT touch dev-health/compose.yml or dev-health/.env (shared, another
#     lane's territory) -- it only adds new, independently-named containers.
#
...
#     The Workbench signs its own web-assertion with this key regardless of
#     which acr-api backend receives it.
#
# Requires (ACR_TARGET=docker only):
#   - The dev-health docker compose stack already up (postgres + clickhouse
#     healthy) -- `docker compose up -d` in dev-health/ if not.
#   - OPENAI_API_KEY in the environment (ACR's model provider is openai;
#     without it acr-api starts but every investigation 503s).
#
# Requires (ACR_TARGET=kiac only):
#   - kubectl on PATH, already pointed at the acr-local kiac cluster (the
#     acr-pilot Helm release's own Secret already carries its model
#     credentials -- this script does not need OPENAI_API_KEY for kiac).
#
# Fixed 2026-08-26 (docker target): investigations were intermittently coming
# back as "ACR could not be reached" / acr_unreachable even though ACR itself
# had completed the work (`docker logs acr-dev-acr-api` showed a real 200).
...
#   The kiac acr-pilot release carries the same coherent pair
#   (requestTimeout=490s / writeTimeout=500s) via its own Helm values.
#
# Usage:
#   bash scripts/dev-launch.sh                       # ACR_TARGET=kiac (default)
#   ACR_TARGET=docker OPENAI_API_KEY=sk-... bash scripts/dev-launch.sh
set -euo pipefail
...
case "$ACR_TARGET" in
  kiac|docker) ;;
  *)
    note "ACR_TARGET must be 'kiac' or 'docker' (got: $ACR_TARGET)"
    exit 1
    ;;
esac
note "ACR_TARGET=$ACR_TARGET"
...
stop_if_ours() {
  # Only stop a container by this name if it was actually built from OUR
  # image -- never blind-stop a same-named container something else created.
  local name="$1" expect_image="$2"
  local img
  img="$(docker inspect "$name" --format '{{.Config.Image}}' 2>/dev/null || true)"
  if [[ -z "$img" ]]; then
    return 0
  fi
  if [[ "$img" != "$expect_image" ]]; then
    note "  $name exists but is not our image ($img) -- not touching it."
    return 0
  fi
...
if [[ "$ACR_TARGET" == kiac ]]; then
  ...
  if ! kubectl -n "$KIAC_NAMESPACE" get svc "$KIAC_SERVICE" >/dev/null 2>&1; then
    note "Can't see svc/$KIAC_SERVICE in namespace $KIAC_NAMESPACE."
    note "  Is kubectl pointed at the acr-local kiac cluster, and is acr-pilot deployed?"
    exit 1
  fi
