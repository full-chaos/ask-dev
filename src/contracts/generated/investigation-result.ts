...
export interface AnswerPlanBudget {
    ...
    /**
     * The declared order a narrowing step takes members in. canonical_id_lexical is ARBITRARY and says so -- what a pre-read guard needs is stability, not relevance. attention_rank exists only after the fact read.
     */
    narrowing_basis: "canonical_id_lexical" | "largest_group_round_robin" | "attention_rank";
}
/**
...
export interface PlanNarrowing {
     ...
     * The declared order a narrowing step takes members in. canonical_id_lexical is ARBITRARY and says so -- what a pre-read guard needs is stability, not relevance. attention_rank exists only after the fact read.
     */
    basis: "canonical_id_lexical" | "largest_group_round_robin" | "attention_rank";
    before: number;
    after: number;
    groups?: boolean;
    /**
...
export interface Cohort {
    ...
    truncated: boolean;
    /**
     * CHAOS-4636: absent on every flat cohort. When present, the cohort-level complete/truncated above are the conjunction/disjunction over these groups, so a reader that ignores groups gets a conservative summary of the whole union rather than a boolean describing only the first group.
     *
     * @maxItems 250
     */
    groups?: CohortGroup[];
