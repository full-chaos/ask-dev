...
export type ACRContextFabricInvestigationResultV1 = {
    ...
    /**
     * CHAOS-5442: names WHY the server refused to act on this question's frame, over a closed vocabulary. ABSENT on every turn that was not refused -- absence means "not refused", never "refused for a reason nobody recorded" (that state is the `unspecified` member). Orthogonal to terminal_reason, which names the CHANNEL an explanation travelled through rather than the decision taken: a refused frame carries both.
     */
    refusal_basis?: "member_kind_unservable" | "frame_invariant_violated" | "unspecified";
};
