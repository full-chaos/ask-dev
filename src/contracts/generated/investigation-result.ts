 ...
 * Source: full-chaos/dev-health-acr @ 31d7dc2b119b92ca21b735fca0632860670baf9c
...
export type ACRContextFabricInvestigationResultV1 = {
    ...
    /**
     * CHAOS-5405 D-d: one record per attempted requirement/origin scope decision. Optional-first: absent on every result written before this field existed, and on any path that resolved no scope at all -- absent and empty are different documents.
     *
     * @maxItems 21
     */
...
export type InterpretedQuestion = {
    ...
    /**
     * CHAOS-5774: the model's own closed-vocabulary classification of what BASIS requested_judgment asks for. Absent means the model made no pick (equivalent to unspecified) -- never derived from requested_judgment's own text by a consumer.
     */
...
export type Cohort = {
    ...
    /**
     * CHAOS-5774: server-computed, true when the investigation's requested judgment asked for a comparison score_meaning does not support. Never set without score_meaning also present.
     */
...
/**
 * One derived requirement: the obligation/role/subject coordinate, what serves it, and -- for a computation -- what that step consumes and whether anything runs it. Derived once before any narrowing, so it says what the answer was PLANNED to contain; the completeness outcome rows say what became of each, joined by the same coordinate string. Carries no member kind: that is written after subject resolution, and these rows are derived before it.
 */
...
export interface CohortGroup {
    ...
    truncated: boolean;
    /**
     * This group's member count AS DISCOVERED when the group was built, before any narrowing. Not adjusted for an upstream discovery-level cap that may have left members belonging to this group undiscovered entirely; that possibility is disclosed via complete and the cohort's own truncated, never by inflating total past what was actually seen.
     */
    total: number;
}
...
export interface RenderShape {
    ...
    /**
     * CHAOS-4415: the CLOSED vocabulary of deterministic selection rules. Exactly one rule produced any given shape; a shape with no rule would be the default-charting this contract forbids.
     */
...
export interface PlanRequirementOutcomeRow {
    ...
    /**
     * What the reader loses. none exactly when the outcome is satisfied or not_applicable; any other outcome carries a real impact.
     */
